...
class AudioPlayer:
    ...
    def display_scrolling_text(self, text, delay=0.1):
        width = 40  # Number of characters to display in one line
        length = len(text)
        text = text + " " * width  # Pad the text with spaces
        for i in range(length + width):
            print(text[i:i + width], end='\r')
            time.sleep(delay)
